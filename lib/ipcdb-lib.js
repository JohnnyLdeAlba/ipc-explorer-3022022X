...
function _epoch_to_time_stamp(epoch)
{
    let time_stamp = new Date(0);
    time_stamp.setUTCSeconds(epoch);
    return time_stamp.toUTCString();
}

function epoch_to_time_stamp(epoch)
{
    let time_stamp = new Date(epoch);
    return time_stamp.toUTCString();
}
...
async function ipcdb_restore_database(database)
{
    ...
    if (session == null)
    {
        console.log("IPCDB_RESTOREDB_FAILED");
...
async function ipcdb_backup_database()
{
    ...
    console.log("IPCDB_BACKUPDB_SUCCESS");
